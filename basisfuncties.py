...
def locate_existing_points(profielen, puntenZ, velden_profielen,trajectlijn,code,velden_uitvoer):
    ...
    # needed_fields = ['OBJECTID', 'SHAPE', 'SHAPE_Length','Shape','Shape_Length',code,toetspeil,'profielnummer']
    for field in existing_fields:
        if field.name not in velden_profielen:
            arcpy.DeleteField_management(profielen, field.name)
    ...
    # select points
    arcpy.MakeFeatureLayer_management(puntenZ, "temp_puntenZ") 
    arcpy.SelectLayerByLocation_management("temp_puntenZ", "INTERSECT", "routes_land", "", "NEW_SELECTION", "NOT_INVERT")
    arcpy.CopyFeatures_management("temp_puntenZ", "punten_land")

    arcpy.MakeFeatureLayer_management(puntenZ, "temp_puntenZ") 
    arcpy.SelectLayerByLocation_management("temp_puntenZ", "INTERSECT", "routes_rivier", "", "NEW_SELECTION", "NOT_INVERT")
    arcpy.CopyFeatures_management("temp_puntenZ", "punten_rivier")
    ...
    velden = ['profielnummer', 'afstand', code, velden_uitvoer]
    keepers = velden
    ...
    arcpy.Merge_management(['punten_land', 'punten_rivier'], 'punten_profielen', fieldmappings)

    arcpy.CalculateField_management("punten_profielen", "afstand", 'round(!afstand!, 2)', "PYTHON")
    ...
    arcpy.CopyFeatures_management('punten_profielen', puntenZ)
    
    print 'Punten op routes gelokaliseerd'
